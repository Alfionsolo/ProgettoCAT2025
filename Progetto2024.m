...
%Finocchiaro Alfio, Traini Francesco, Panaroni Davide, Xhika Kleolin
clear; clc;
...
k = 20;
J0 = 2;
Ji = [0.5, 0.01, 0.01, 0.08];
i = 1:4;
J = @(theta) J0 + sum(Ji .* cos(i * theta + psi));
%% Punto 1 - Linearizzazione

dJ = @(th) sum(-i .* Ji .* sin(th * i + psi));
df2dx1 = - k/J(x_e(1)) + ((-u_e + beta*x_e(2) + k*x_e(1))*dJ(x_e(1)))/(J(x_e(1))^2);

%linearizzazione nell'intorno di equilibrio
A = [0, 1; df2dx1, -beta/J(x_e(1))];
B = [0; 1/J(x_e(1))];
C = [1 0];
D = 0;
